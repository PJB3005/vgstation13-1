...
/obj/mecha/Topic(href, href_list)
	...
	if(usr.stat != 0 || (usr.status_flags & FAKEDEATH))
		return
	var/datum/topic_input/_filter = new /datum/topic_input(href,href_list)
	if(href_list["select_equip"])
		if(usr != src.occupant)	return
		var/obj/item/mecha_parts/mecha_equipment/equip = _filter.getObj("select_equip")
		if(equip)
			src.selected = equip
	...
	if(href_list["rfreq"])
		if(usr != src.occupant)	return
		var/new_frequency = (radio.frequency + _filter.getNum("rfreq"))
	...
	if(href_list["req_access"] && add_req_access)
		...
		output_access_dialog(_filter.getObj("id_card"),_filter.getMob("user"))
		return
	if(href_list["maint_access"] && maint_access)
		if(!in_range(src, usr))	return
		var/mob/user = _filter.getMob("user")
		if(user)
			...
			output_maintenance_dialog(_filter.getObj("id_card"),user)
		return
	...
	if(href_list["add_req_access"] && add_req_access && _filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access += _filter.getNum("add_req_access")
		output_access_dialog(_filter.getObj("id_card"),_filter.getMob("user"))
		return
	if(href_list["del_req_access"] && add_req_access && _filter.getObj("id_card"))
		if(!in_range(src, usr))	return
		operation_req_access -= _filter.getNum("del_req_access")
		output_access_dialog(_filter.getObj("id_card"),_filter.getMob("user"))
		return
