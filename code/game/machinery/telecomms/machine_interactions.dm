...
/obj/machinery/telecomms/multitool_menu(var/mob/user,var/obj/item/device/multitool/P)
	...
	var/dat = {"
		...
		<p><b>Integrity:</b> [Clamp(integrity, 0, MAX_INTEGRITY)]%</p>
	"}
	if(on && toggled)
		dat += {"
		...
		"}
		if(hide)
			dat += "<p><b>Shadow Link:</b> ACTIVE</p>"
			...
			*/
			...
			if("network")
				...
				if(newnet && canAccess(usr))
					if(length(newnet) > 15)
						temp = "<font color = #666633>-% Too many characters in new network tag (max 15) %-</font color>"
